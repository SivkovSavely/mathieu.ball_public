...
export default {
    trigger: /^(?:залго)/i,
    name: 'залго',
    description: 'добавляет страшные символы.',
    ...
    async handler(trigger: string, args: string[], ctx: BotMsgCtx) {
        if (args.length === 0 && !ctx.hasReplyMessage && !ctx.hasForwards) {
            await ctx.replyError('введи сообщение после команды так: матье залго ghbdtn vbh!\n' +
                'либо перешли сообщение и введи просто "матье залго"');
            return;
        }
...
function rand_zalgo(array: string[])
{
    var ind = Math.floor(Math.random() * array.length);
    return array[ind];
}

function is_zalgo_char(c: string)
{
    let i: number;
...
function convert(original: string): string {
    //return [...zalgo_up, ...zalgo_mid, ...zalgo_down].map(x => `a${x} --> ${x} --> ${x.codePointAt(0)}`).join('\n');
    ...
    for (let i=0; i<txt.length; i++)
    {
        ...
        for (let j=0; j<num_up; j++)
            converted += rand_zalgo(zalgo_up);
        for (let j=0; j<num_mid; j++)
            converted += rand_zalgo(zalgo_mid);
        for (let j=0; j<num_down; j++)
            converted += rand_zalgo(zalgo_down);
    }
